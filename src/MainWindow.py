...
class MainWindow(QMainWindow, Ui_MainWindow):
    """
    GUI-only variant: Importer/starter ikke hardware.
    Når du vil aktivere devices igen, sæt gui_only=False i main.py.
    """
    version = 'V1.04.13'
    ...
    def __init__(self, app, gui_only: bool = False):
        ...
        # App-data paths og logging
        self.app_paths = AppDataPaths('mlm2pro-gspro-connect')
        self.app_paths.setup()
        self.__setup_logging()

        # Settings + GSPro + Putting
        self.settings = Settings(self.app_paths)
        self.gspro_connection = GSProConnection(self)  # forventer shot_sent slot
    ...
    def __setup_logging(self):
        ...
        # (valgfrit) log .traineddata filer i current working dir
        cwd = os.getcwd()
    ...
    def __setup_ui(self):
        # Launch monitor i GUI-only mode
    ...
    # -------------------------------------
    # Auto-start (kun sikre ting i GUI-only)
    # -------------------------------------

    def __auto_start(self):
        """
        I GUI-only mode starter vi ikke hardware. GSPro kan auto-startes hvis sat i settings.
        """
        if getattr(self.settings, 'auto_start_all_apps', 'No') == 'Yes':
            gspro_path = getattr(self.settings, 'gspro_path', '')
            if len(gspro_path) > 0 and os.path.exists(gspro_path):
                # Nogle builds bruger 'grspo_window_name' – derfor hasattr check
                if hasattr(self.settings, 'grspo_window_name'):
                    self.log_message(LogMessageTypes.LOG_WINDOW, LogMessageSystems.CONNECTOR, 'Starting GSPro')
                    self.gspro_connection.gspro_start(self.settings, True)

            # Putting timer/tilstand er harmløst i GUI-only
            self.putting.putting_stop_start()

    # -----------------------
    # Launch monitor (GUI-only)
    # -----------------------
    ...
    def __setup_launch_monitor(self):
        """
        I GUI-only mode bruger vi altid screenshot-monitor (ingen hardware/BLE/sockets).
        Når du vil aktivere enheder igen, sæt gui_only=False i main.py – hardware importeres lazy.
        """
        ...
        if self.gui_only:
            ...
            self.launch_monitor_groupbox.setTitle("GUI Only Mode (Screenshot)")
            return

        # -------- Ikke GUI-only: lazy imports (kører ikke i GUI-only) --------
        device_id = getattr(self.settings, 'device_id', None)
        if device_id == LaunchMonitor.RELAY_SERVER:
        ...
        elif device_id == LaunchMonitor.MLM2PRO_BT:
            ...
            self.server_control_widget.show()
            self.actionDevices.setEnabled(False)
        elif device_id == LaunchMonitor.R10_BT:
            ...
            # Fallback til screenshot-mode hvis intet valgt
            self.launch_monitor = DeviceLaunchMonitorScreenshot(self)
            self.device_control_widget.show()
            self.server_control_widget.hide()
            self.actionDevices.setEnabled(True)

        self.launch_monitor_groupbox.setTitle(f"{device_id} Launch Monitor")

    # -----------------------
    # Knap/menupunkt-handlers
    # -----------------------
    ...
    def closeEvent(self, event: QShowEvent) -> None:
        ...
        logging.debug(f'{MainWindow.app_name} Closing launch monitor')
        if self.launch_monitor:
            self.launch_monitor.shutdown()
    ...
    def __donate(self):
        webbrowser.open("https://ko-fi.com/springbok_dev", new=2)

    def __shop(self):
        webbrowser.open("https://cascadia3dpd.com", new=2)
    ...
    def __add_shot_history_row(self, balldata: BallData):
        ...
        result = 'Success' if balldata.good_shot else 'Failure'
    ...
    # -----------------------
    # Slots fra GSProConnection
    # -----------------------

    def shot_sent(self, balldata):
        """Slot kaldt fra GSProConnection, når et skud er sendt/klargjort."""
        self.__add_shot_history_row(balldata)

    def __shot_history_changed(self):
        """Opdater højre sidefelter når valg i shot-history ændres."""
        i = 1
        for metric in BallData.properties:
